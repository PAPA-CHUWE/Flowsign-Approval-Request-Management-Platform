...
import type { StatusKey } from "@/components/tickets/StatusDropdown";
...
// ─── TicketList ───────────────────────────────────────────────────────────────
type SortField = "reference" | "priority" | "submittedAt" | "completionDate";

function TicketList({ tickets }: { tickets: MockTicket[] }) {
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      ...
      {/* ── Table ── */}
      <div className="w-full bg-white border border-[#E8E6DE] rounded-[12px]
                      overflow-hidden shadow-[0_1px_6px_rgba(0,0,0,0.04)]">

        {/* Header */}
        <div className="flex items-center gap-3 px-4 py-3
                        border-b border-[#E8E6DE] bg-[#FAFAF8]">
          {cols.map((col) => (
            <div
              key={col.label}
              className={cn(
            ...
            >
              <span className="text-[11px] font-semibold text-[#888780]
            ...
            >
              ...
              <div className="w-[130px] shrink-0">
                <StatusDropdown
                  ...
                  onChange={(v) =>
                    setStatuses((prev) => ({ ...prev, [ticket.id]: v }))
                  }
                />
              </div>
              ...
              <div className="w-[120px] shrink-0">
                <p className="text-[11px] text-[#888780]">
                  {ticket.completionDate ? formatDate(ticket.completionDate) : "—"}
                </p>
              </div>
            </div>
          ))
