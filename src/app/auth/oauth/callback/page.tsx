...
function Spinner() {
  return (
    <svg className="animate-spin h-8 w-8 text-[#1D9E75]" viewBox="0 0 24 24" fill="none" aria-hidden="true">
...
function OAuthCallbackContent() {
  ...
  useEffect(() => {
    ...
    // Store token so apiClient sends it as Authorization: Bearer
    ...
    return (
      <div className="min-h-screen w-full bg-[#F1EFE8] flex items-center justify-center px-4">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-[#E8E6DE] p-8 text-center">
          ...
          <h1 className="font-serif text-[22px] font-normal text-[#2C2C2A] tracking-[-0.02em] mb-2">Sign-in failed</h1>
          <p className="text-[13px] text-[#5F5E5A] leading-relaxed mb-6">{errorMessage}</p>
          <button
